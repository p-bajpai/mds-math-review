...
$$
\sum_{\text{all }(x_0,y_0)\text{ in } \mathcal{X}} \big( \; (mx_0+b) - y_0 \; \big)^2 .
$$

The symbol $\sum$ denotes a sum, and the text below the symbol tells us which values we are summing over. In this case, we sum the individual contributions coming from each point in our dataset.
:::
...
$$
\nabla SSE (m,b) \quad = \quad  \begin{bmatrix} \displaystyle\sum_{\text{all }(x_0,y_0)\text{ in } \mathcal{X}} 2x_0(mx_0 + b - y_0)  \\  \displaystyle\sum_{\text{all }(x_0,y_0)\text{ in } \mathcal{X}} 2(mx_0+b-y_0) \end{bmatrix}. 
$$
...
$$
\nabla SSE (m,b) \quad = \quad  \begin{bmatrix} \displaystyle\sum_{(x_0,y_0)\in \mathcal{X}} 2x_0(- y_0)  \\  \displaystyle\sum_{(x_0,y_0)\in \mathcal{X}} 2(-y_0) \end{bmatrix}. 
$$

Our table contains the values for $(x_0,y_0)$ that we need to sum over. These give us

$$
\sum_{(x_0,y_0)\in \mathcal{X}} 2x_0(- y_0) = -2 \big(1(2.5)+2(4.7)+3(6.33)+4(8.09)+5(11)\big)= -236.5
$$

and
$$
\sum_{(x_0,y_0)\in \mathcal{X}} 2(- y_0) = -2 \big( 2.5+4.7+6.33+8.09+11 \big) = -65.24.
$$
...
The function $f(m,b) = -236.5m - 65.24b$ meets this criterion.

**Note:** This function is not actually needed for solving the least squares problem.
:::
...
**Note:** Again, this function is not needed for solving the least squares problem. It is the two-variable equivalent to the "tangent line to the graph" that you likely saw in a typical one-variable calculus class.
