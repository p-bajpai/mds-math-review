...
$$
f([x_1,x_2]^T) = e^{-\frac12(4x_1^2 + \frac12 x_2^2)}.
$$
...
d) For each choice of $x_2 = -2, -1, 0, 1, 2$, simplify $f$ into a function of the single variable $x_1$. Then, plot each of the resulting functions using your favourite graphing software. What do you notice about the graphs?
...
We could repeat this process fixing instead $x_1 = -2, -1, 0, 1, 2$ in turn, and plotting the resulting function of $x_2$. This results in a similar picture, except the graph changes more dramatically across different values of $x_1$.

The graph of $f$, as a function of 2 variables, looks like a "hill" with a peak at the origin; from this peak the graph shows smooth exponential decay in all directions. The decay is faster in the $x_1$ direction (corresponding to the larger value $4$ in $A$) than in the $x_2$ direction (which corresponds to the smaller value $1/2$). In other words, the hill is narrow in the $x_1$ direction and wide in the $x_2$ direction.
...
Points randomly sampled from a two-variable Gaussian with mean $[0,0]$ and covariance matrix given by $B^{-1}$. The probability density function of the sampling distribution is proportional to the function $g(\bar x)$ from above.
:::
</center>
<br><br>
